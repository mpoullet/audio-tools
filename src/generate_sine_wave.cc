...
public:
    explicit OptionName (std::string const& long_name,
                         std::string const& short_name) : long_name_ (long_name),
...
{
    if (audio_format == "pcm32")
        return SF_FORMAT_PCM_32;
    if (audio_format == "pcm24")
        return SF_FORMAT_PCM_24;
    if (audio_format == "pcm16")
        return SF_FORMAT_PCM_16;
    if (audio_format == "float")
        return SF_FORMAT_FLOAT;
    if (audio_format == "double")
...
int main (int argc, char** argv)
{
    try
    {
        ...
        desc.add_options () (parameters::frequencyOptionName.get_complete ().c_str (), po::value<parameters::HumanEarableFrequency> ()->value_name (" ")->required (), "Hz") (parameters::sampleRateOptionName.get_complete ().c_str (), po::value<parameters::SampleRate> ()->value_name (" "), sampleRatesOptions.c_str ()) (parameters::audioFormatOptionName.get_complete ().c_str (), po::value<parameters::AudioFormat> ()->value_name (" "), validAudioFormats.c_str ()) (parameters::prefixNameOptionName.get_complete ().c_str (), po::value<parameters::PrefixName> ()->value_name (" "), "<prefix_name>_<sample-rate>_<audio_format>.wav") (parameters::durationOptionName.get_complete ().c_str (), po::value<parameters::Duration> ()->value_name (" "), "length of the sine wave in seconds") (parameters::verboseOptionName.get_complete ().c_str (), po::bool_switch ()->default_value (false), "print each samples") (parameters::helpOptionName.get_complete ().c_str (), "");
